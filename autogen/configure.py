...
class ConfigureScriptGenerator():
  def __init__( self, project, path, version, install = True, static = True ):
    ...
    if os.path.exists( self.__path + "/LICENSES/GPL-2.0-only.txt" ):
      self.__freeLicense = "GPL-2.0"
      self.__licenseName = "GNU General Public License Version 2"
    elif os.path.exists( self.__path + "/LICENSES/GPL-3.0-only.txt" ):
      self.__freeLicense = "GPL-3.0"
      self.__licenseName = "GNU General Public License Version 3"
    elif os.path.exists( self.__path + "/LICENSES/LGPL-2.1-only.txt" ):
      self.__freeLicense = "LGPL-2.1"
      self.__licenseName = "GNU Lesser General Public License Version 2.1"
    elif os.path.exists( self.__path + "/LICENSES/LGPL-3.0-only.txt" ):
      self.__freeLicense = "LGPL-3.0"
      self.__licenseName = "GNU Lesser General Public License Version 3"
    else:
			print "no free licenses (LICENSES/GPL-2.0-only.txt, etc.) could be found"
			sys.exit( 1 )
  ...
  def __replaceValues( self, value ):
    ...
    value = value.replace( "@PRODUCT_LICENSE_FREE_NAME@", self.__licenseName )
    return value
